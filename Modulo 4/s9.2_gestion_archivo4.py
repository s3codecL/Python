...
def reemplazar_contenido_archivo():
    nombre_archivo = "informacion.dat"
    contenido_reemplazo = "Datos de Procesamiento"

    try:
        with open(nombre_archivo, "r") as archivo:
            contenido_actual = archivo.read()
        
        contenido_nuevo = contenido_actual.replace("Datos de Información", contenido_reemplazo)
        
        with open(nombre_archivo, "w") as archivo:
            archivo.write(contenido_nuevo)
        
        print("Se realizaron:", contenido_actual.count("Datos de Información"), "reemplazos.")
